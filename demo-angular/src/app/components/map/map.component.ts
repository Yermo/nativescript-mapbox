...
* USAGE:
*
* <map access_token="mapbox access token"
*   style="mapbox style"
*   fallbackLatitude="" 
*   fallbackLongitude="">
...
* it crashes. The workaround is to destroy and recreate the map on each pause/resume
* iteration.
*/
...
export class MapComponent implements OnInit, OnDestroy {
  ...
  */
  ...
  registerEventHandlers() {
    ...
    // we need to do the same kind of thing for page navigation events

    this.page.on( 'navigatingTo', ( data ) => {

      console.log( "MapComponent:registerEventHandlers(): navigating to page" );
    ...
    this.page.on( 'navigatingFrom', ( data ) => {
      ...
      console.log( "MapComponent:registerEventHandlers(): navigating from page" );

      // onPause catches the case where the is no map.
  ...
  */
  ...
  ngOnInit() {
    console.log( "mapComponent constructor() with style '" + this.style + "' and access token '" + this.access_token + "'" );
  }
  ...
  */
  ...
  async onPause() {
    ...
    // under iOS pause resume resinertion does not seem to work.

    if ( this.platform.is( "android" ) && this.mapboxView ) {

      console.log( "MapComponent:onPause(): removing map" );

      await this.saveMapState();

      console.log( "MapComponent:onPause(): state saved" );

      await this.mapboxView.destroy();

      console.log( "MapComponent:onPause(): map destroyed" );

      this.shown = false;

      // reset the ready promise so that things are coture when we
      // are resumed.
  ...
  */
  ...
  async saveMapState() {
    ...
    this.settingsService.set( 'mapSettings', settings );
...

{
  ...
  addTestLine() {
    ...
    console.log( "MapComponent:addTestLine(): Adding test line" );
    ...
    this.mapboxView.addLayer({
      ...
      console.error("MapComponent:addTestLine(): addTestLine threw an error:", error );
    });

    this.mapboxView.onMapEvent( 'click', 'testLine', ( point ) => {

      console.log( "MapComponent:addTestLine(): line clicked!!" );
  ...
  addTestLinePoint() {
    ...
    console.log( "Mapbox:addTestLinePoint(): before adding line point" );

    this.mapboxView.addLinePoint( 'testLine', [ -76.926164, 39.019062 ] );
  ...
  addTestPolyline() {
    ...
    this.mapboxView.addPolyline( options );
